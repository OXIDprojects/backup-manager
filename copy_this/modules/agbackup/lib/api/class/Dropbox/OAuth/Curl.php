...
class Dropbox_OAuth_Curl extends Dropbox_OAuth {
    ...
    public function fetch($uri, $arguments = array(), $method = 'GET', $httpHeaders = array()) {
		...
		if (is_string($arguments) && strtoupper($method) == 'POST') {
        ...
        } else if (is_string($arguments) && strtoupper($method) == 'PUT') {
            $httpHeaders=array_merge($httpHeaders,$this->getOAuthHeader($uri, array(), $method));
		} else {
		...
		if (strtoupper($method) == 'POST') {
            ...
            $fp = fopen('php://temp/maxmemory:'. strlen($arguments), 'w');
            if (!$fp) {
            ...
            curl_setopt($ch, CURLOPT_INFILESIZE, strlen($arguments));
		} else {
    ...
    private function getOAuthHeader($uri, $params, $method = 'GET', $oAuthParams = null) {
        ...
        $tokens = $this->getToken();
        $hash = $this->consumerSecret.'&'.$tokens['token_secret']; //$this->hash_hmac_sha($this->consumerSecret.'&'.$tokens['token_secret'], $baseString);
        $signature = $hash;//base64_encode($hash);
