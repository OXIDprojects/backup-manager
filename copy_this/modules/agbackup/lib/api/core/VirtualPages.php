...
class VirtualPages
{
	...
	public static function SetPath($path = null)
	{
		if ( $path === null )
		{
			if ( !isset($_SERVER["REQUEST_URI"]) )
				...
				$path = preg_replace("/\?(.*)$/", '', preg_replace("#^". Storm::$RelativePath ."(/?index\.php)?/#", '', $_SERVER["REQUEST_URI"]));
		}

		self::$PagePath = self::ProcessRewrites(ltrim($path, '/'));
	...
	/**
	 * Returns the base URL for http/https
	 */
