...
class Dropbox_API {
    ...
    /**
     * Uploads a new file using the chunked upload
     *
     * @param string $path Target path (including filename) 
     * @param string $file Either a path to a file or a stream resource 
     * @param string $root Use this to override the default root path (sandbox/dropbox)  
     * @return bool 
     */
    public function putFileChunked($path, $file, $root = null) {
        ...
        $response = $this->chunkedFetch($this->api_content_url . 'chunked_upload', $file, 0, 4 * 1024 * 1024);
        ...
        while (!feof($file)) {
            ...
            $response = $this->chunkedFetch($this->api_content_url . 'chunked_upload', $file, $offset, 10 * 1024 * 1024, $data['upload_id']);
            $data = json_decode($response['body'], true);
        ...
        $response = $this->oauth->fetch($this->api_content_url . 'commit_chunked_upload/' . $root . '/' . $path, array('upload_id' => $data['upload_id']), 'POST', array());
    ...
    */
    ...
    /**
     * This method is used to generate PUT requests for chunked file upload 
     * 
     * @param string $uri 
     * @param array $arguments 
     * @return bool 
     */
	protected function chunkedFetch($uri, $file, $offset, $length, $upload_id = null) {
        $data=fread($file, $length);

        $headers=array();

        if ($upload_id !== null)
            $uri.='?upload_id=' . $upload_id . '&offset=' . $offset;
        else
            $uri.='?offset=' . $offset;

        return $this->oauth->fetch($uri, $data, 'PUT', $headers);
